...
- **MIST-3** Events 518 and 519 have one occurrence each in capture-52 with the same entity id (2577), suggesting a spawn plus state-change pair. Semantics unknown. Event 523 `NewMistsWispSpawn` now routed to `MistsWispHandler` and rendered with the shared `mist_0` fallback image (no dedicated `wisp_sign.webp` asset yet), but carries no rarity; the in-game tooltip on a feu follet does show rarity so the info exists somewhere and needs targeted live capture with state interactions.
...
- **MIST-5** Dedicated `wisp_sign.webp` asset is missing from `web/images/Resources/`. Facet 3 falls back to `mist_0.webp` in `MistsWispDrawing.invalidate` until a proper asset is committed. Users will see feu follet markers indistinguishable from E0 mist portals until the asset is added.
