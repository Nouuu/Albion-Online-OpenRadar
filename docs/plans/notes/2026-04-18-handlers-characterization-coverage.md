...
- 2026-04-26 #92 typeId OFFSET drift + spurious tier shift resolved. Cross-validated 6469 NewMob events in 8 pcaps and 5889 events across 47 session logs against `mobs.min.json`: every wire HP resolves at OFFSET=16, zero outliers at any other offset. The previous OFFSET=15 was a deduction from DEATHEYE C# code never re-anchored on wire HP after upstream insertions; the legacy `LivingResourceTier` `t-1` shift cancelled the +1 drift on living non-DYNAMIC/non-DEAD entries by accidental compensation, but DEAD/DYNAMIC variants surfaced it (T7 fiber DEAD displayed T7 instead of T6, T7 fiber alive displayed T6 instead of T5). Fix: bump `MobsDatabase.OFFSET` from 15 to 16 and reduce `getLivingHarvestTier` to `mob?.t ?? 0` (combat tier passthrough; upstream `mobs.xml` `Loot.Harvestable @tier` equals the Mob `@tier` on every checked entry). Removed the 20-entry `MIN_TIER_BY_TYPE` table that had silently missed `*_CRITTER_ROADS_VETERAN` and `*_CRITTER_ROADS_ELITE`. Diagnostic plumbing kept: `mob.uniqueName` exposed by `MobsHandler` for the Resources page `Show DB Name` overlay (`settingLivingResourcesName`) and `CritterCorpseTierAudit` log emitted by `HarvestablesHandler` on each critter corpse spawn. New `tools/offset-validate` Go binary parses any pcap set and prints `(typeId, hp, ap, ms)` JSONL for offset re-anchoring at each upstream DB refresh. Tests: 24 unit tests on `getLivingHarvestTier` (passthrough, all variants), 7 derived `roads veteran/elite` tests, 3 `issue #92` regression tests (wire 374/526/535), 62-row synthetic coverage grid rebuilt from canonical uniqueNames at OFFSET=16, 10 pcap-derived integration tests with consolidated `uniqueName` assertions. Fixes #92.
