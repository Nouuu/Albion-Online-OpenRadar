...
- `app.js` – Entry point, Express server, WebSocket server, packet capture wiring.
- `scripts/classes/` – Core classes: packet parser, handlers, utilities.
- `scripts/Handlers/` – Domain-specific handlers (mobs, resources, players, etc.).
- `scripts/Drawings/` – Rendering logic for the radar (shapes, markers, overlays).
- `scripts/Utils/` – Shared helpers (logging, settings, constants, etc.).
- `views/` – EJS templates for the UI.
- `server-scripts/` – Server-side helpers (adapter selection, logging server, etc.).
- `work/` – Python tooling and data for offline analysis (TypeIDs, dumps, etc.).
...
- ⚠ URL bar is visible (cannot be fully hidden).
- ⚠ Window borders depend on the browser/OS.
...
- See `docs/project/IMPROVEMENTS.md` and `docs/project/PLAYER_DETECTION_STATUS.md` for investigation history.

Static reference files generated by Python tools (under `tools/output/`):

- `harvestables-typeids.js` – Static harvestable TypeIDs.
- `living-resources-enhanced.json` – 225 living creatures with metadata.
- `living-resources-reference.js` – JavaScript-ready reference.
...
- **93 animals** (LivingSkinnable – Hide)
- **46 Fiber guardians**
- **43 Wood guardians**
- **43 Ore guardians**
- **43 Rock guardians**
...
- `tools/output/living-resources-enhanced.json` – 225 creatures with metadata.
- `tools/output/living-resources-reference.js` – JavaScript module ready to use.
...
- `T4_MOB_CRITTER_HIDE_MISTCOUGAR` → HP 962 → ~1.4 ratio → .1
...
1. `tools/parse-all-resources.py` – Extract static resources ✅
2. `tools/analyze-missing-typeids.py` – Coverage analysis for `MobsInfo.js` ✅
3. `tools/search-living-mobs.py` – Exhaustive search of ID fields ✅
4. `tools/extract-mob-metadata.py` – Extract living resource metadata ✅

### 5.2 Data Files to Keep

- `tools/output/harvestables-typeids.js` – 139 static TypeIDs.
- `tools/output/living-resources-enhanced.json` – 225 creatures with metadata.
- `tools/output/living-resources-reference.js` – JavaScript reference module.
...
**Estimated effort:** 2–4 hours of gameplay to collect ~100–150 missing TypeIDs.
...
|----------|-----------|----------------------|---------------|
| Hide     | 85 IDs    | 0 IDs                | ✓ Full base   |
| Fiber    | 39 IDs    | 0 IDs                | ✓ Full base   |
| Ore      | 38 IDs    | 0 IDs                | ⚠ Partial     |
| Rock     | 35 IDs    | 0 IDs                | ⚠ Partial     |
| Logs     | 0 IDs     | 0 IDs                | ❌ Missing    |

**Critical missing (~50–60 TypeIDs):**

- Hide T4–T5 .1/.2/.3 (3×2 tiers = 6 variants).
- Fiber T4–T5 .1/.2/.3 (6 variants).
- Wood guardians (T3–T8, all variants).
- Enchanted Ore/Rock guardians.

### 5.5 Proposed Implementation Plan

#### Phase 1: Validation (1–2h) – OPTIONAL

- [ ] Add HP validation in `MobsHandler.js`.
- [ ] Improve logging with expected-creature hints.
- [ ] Test against existing TypeIDs.

#### Phase 2: UI Integration (2–3h) – OPTIONAL

- [ ] Add creature guide to `resources.ejs`.
- [ ] Implement filters by animal.
- [ ] Display metadata on the radar.

#### Phase 3: Enrichment (2–3h) – OPTIONAL
