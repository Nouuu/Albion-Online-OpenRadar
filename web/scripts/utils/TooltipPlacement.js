...
export function placeTooltip(icon, size, clip, margin = MARGIN) {
    ...
    const above = icon.top - margin - clip.top;
    const below = clip.bottom - icon.top - icon.height - margin;
    return {nudge: fitted - left, below: above < size.height && below > above};
}
...
function place(target) {
    ...
    const top = box.top + (content.clientTop || 0);
    const clip = {left, right: left + content.clientWidth, top, bottom: top + content.clientHeight};
